...
let mappings = [[src, dest];
    ...
    [./homemanager-modules/configs/nushell/config.nu, ~/.config/nushell/config.nu],
    [./homemanager-modules/configs/nushell/env.nu, ~/.config/nushell/env.nu],
    [./homemanager-modules/configs/nushell/libs, ~/.config/nushell/libs],
    [./homemanager-modules/configs/nushell/login.nu, ~/.config/nushell/login.nu],
]

def setup [] {
    for map in $mappings {
        let src_path = $map.src | path expand
        let dest_path = $map.dest | path expand -n
        ...
        # check if it is already linked in that case skip it
        if ($dest_path | path exists ) {
            if (( ls -lD $dest_path | get -i 0.target ) == $src_path)  {
                std log debug $"($src_path) is already linked to ($dest_path), skipping"
            } else {
                std log error $"($dest_path) already exists remove and run the setup again"
            }
            continue
        }
        ...
        try {
            ...
            std log error $"Couldn't link to ($dest_path), possible that file already exists"
        }
    }
