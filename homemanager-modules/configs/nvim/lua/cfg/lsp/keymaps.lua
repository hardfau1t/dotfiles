...
mod.setup = function(client, bufnr)
    ...
    vim.keymap.set('n', "<leader>li", toggle_inlay_hints, { buffer = bufnr, desc = "restart lsp server"})
    ...
    vim.keymap.set('n', "<leader>lI", "<cmd>LspInfo<CR>", { buffer = bufnr, desc = "Lsp status"})
    ...
    vim.keymap.set('n', "<leader>dd", function () vim.diagnostic.enable(false) end, { buffer = bufnr , desc = "disable lsp diagnostics"})
